<script lang="ts">
  ...
  const mockProgressData = {
    ...
    outstanding_tasks: [
      {
        id: "1",
        description: "Wig styling for Sailor Moon",
        due_date: "2025-10-23",
        priority: "high",
      },
      ...
      <div class="grid grid-cols-2 gap-4">
        {#each loadingPlaceholders as placeholder (placeholder)}
          <div class="text-center">
            <div
              class="h-6 rounded w-8 mx-auto mb-1"
              style="background: var(--theme-sidebar-border);"
            ></div>
            <div
              class="h-3 rounded w-16 mx-auto"
              style="background: var(--theme-sidebar-border);"
            ></div>
          </div>
        {/each}
  ...
  {:else if progressData}
    ...
    <div class="space-y-3">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-2">
          <span class="text-xs font-medium" style="color: var(--theme-sidebar-text); opacity: 0.7;">
            Team: {teamId}
          </span>
          <span
            class="text-sm font-bold"
            style={getProgressTextColor(progressData.overall_progress)}
          >
    ...
    {#if widget.settings?.showDetails !== false}
      ...
      {#if progressData.outstanding_tasks.length > 0}
        <div class="pt-3" style="border-top: 1px solid var(--theme-sidebar-border);">
          <div class="flex items-center justify-between mb-2">
            <span class="text-xs font-medium" style="color: var(--theme-foreground);"
              >Outstanding Tasks</span
            >
            <span class="text-xs" style="color: var(--theme-sidebar-text); opacity: 0.6;"
              >{progressData.outstanding_tasks.length}</span
            >
          </div>
          ...
          <div class="space-y-1">
            {#each progressData.outstanding_tasks.slice(0, 3) as task}
              <div class="flex items-center justify-between text-xs">
                <span class="truncate pr-2" style="color: var(--theme-sidebar-text); opacity: 0.8;"
      ...
      {#if progressData.recent_updates.length > 0}
        <div class="pt-3" style="border-top: 1px solid var(--theme-sidebar-border);">
          <div class="text-xs font-medium mb-2" style="color: var(--theme-foreground);">
            Recent Updates
          </div>
          <div class="space-y-1">
            {#each progressData.recent_updates.slice(0, 2) as update}
              <div class="flex items-start space-x-2 text-xs">
                ...
                ></div>
                <div class="flex-1">
                  <span style="color: var(--theme-sidebar-text); opacity: 0.8;"
                    >{update.description}</span
                  >
                  <div class="text-xs" style="color: var(--theme-sidebar-text); opacity: 0.5;">
      ...
      <div class="flex justify-between text-xs gap-2">
        {#each progressData.categories.slice(0, 4) as category}
          <div class="text-center flex-1">
            ...
            <div class="text-xs mb-1" style="color: var(--theme-sidebar-text); opacity: 0.7;">
              {category.name}
            </div>
            <div
