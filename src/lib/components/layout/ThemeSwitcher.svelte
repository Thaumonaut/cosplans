...
>
	<div class="p-1 max-h-[400px] overflow-y-auto">
		<!-- Light Themes -->
		<div class="px-2 py-1.5 text-xs font-semibold uppercase tracking-wider" style="color: var(--theme-sidebar-muted);">
			Light Themes
		</div>
		{#each lightThemes as themeVariant (themeVariant.id)}
			<button
				type="button"
				class="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold transition-colors {$theme.activeId === themeVariant.id
					? 'bg-[var(--theme-sidebar-active)]'
					: 'hover:bg-[var(--theme-sidebar-hover)]'}"
				style={$theme.activeId === themeVariant.id ? 'color: var(--theme-sidebar-accent);' : 'color: var(--theme-foreground);'}
		...
		<!-- Dark Themes -->
		<div class="mt-2 px-2 py-1.5 text-xs font-semibold uppercase tracking-wider border-t border-[var(--theme-sidebar-border)]" style="color: var(--theme-sidebar-muted);">
			Dark Themes
		</div>
		...
		{#if customTheme}
			...
			<button
				type="button"
				class="flex w-full items-center gap-3 rounded-md px-3 py-2 text-sm font-semibold transition-colors {$theme.activeId === 'custom'
					? 'bg-[var(--theme-sidebar-active)]'
					: 'hover:bg-[var(--theme-sidebar-hover)]'}"
				style={$theme.activeId === 'custom' ? 'color: var(--theme-sidebar-accent);' : 'color: var(--theme-foreground);'}
		...
		<div class="mt-2 border-t border-[var(--theme-sidebar-border)] pt-1">
			...
			>
				<LucideIcon name="Palette" size={16} />
				<span class="flex-1 text-left">{customTheme ? 'Edit Custom Theme' : 'Create Custom Theme'}</span>
			</button>
		</div>
	</div>
</Dropdown>
